    ...
    try {
      ...
      await api.post(
        "/Kyc/submit",
        {
          ...
          addressProof: `${formData.address}, ${formData.city}, ${formData.state}, ${formData.zipCode}`
        },
        {
          headers: {
      ...
      // 2️⃣ Fetch KYC status from backend
      const kycResponse = await api.get("/Kyc/me");

      const { status, submittedAt, adminRemarks } = kycResponse.data;
      ...
      const normalizedStatus =
        statusMap[status.toLowerCase()] ?? "not_started";

      localStorage.setItem("kycStatus", normalizedStatus);
      localStorage.setItem("kycSubmittedAt", submittedAt);
      localStorage.setItem("kycAdminRemarks", adminRemarks || "");
      ...
      console.error("KYC submission failed:", error.response);
      alert(error.response?.data || "KYC submission failed");
    }
  };
                ...
                <div className="w-64 bg-gray-100 rounded-xl p-6 flex-shrink-0">
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold ${currentStep >= 1 ? 'bg-blue-500' : 'bg-gray-400'
                        }`}>
                        1
                      </div>
                      <div>
                    ...
                    {/* Step 2 */}
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold ${currentStep >= 2 ? 'bg-blue-500' : 'bg-gray-400'
                        }`}>
                        2
                      </div>
                      <div>
                    ...
                    {/* Step 3 */}
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold ${currentStep >= 3 ? 'bg-blue-500' : 'bg-gray-400'
                        }`}>
                        3
                      </div>
                      <div>
                    ...
                    {/* Step 4 */}
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold ${currentStep >= 4 ? 'bg-blue-500' : 'bg-gray-400'
                        }`}>
                        4
                      </div>
                      <div>
                ...
                <div className="flex-1 bg-white rounded-xl border border-gray-200 p-8">
                  ...
                  <div className="flex justify-between mt-8 pt-6 border-t">
                    <button
                      ...
                      className={`px-8 py-3 rounded-lg font-semibold transition ${currentStep === 1
